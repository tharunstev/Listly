...
const Home = ({ input }) => {
  ...
  const handleEditTask = (taskId, updatedTodo) => {
    dispatch(updateTask({ taskId, completed: selectedTask.completed, todo: updatedTodo }))
      .then(() => {
  ...
  return (
    <Container>
      <ButtonsContainer>
        {filter !== 'all' && (
          <WhiteButton variant="outlined" onClick={() => taskTypeHandler('all')}>Tasks</WhiteButton>
        )}
        {filter !== 'completed' && (
          <WhiteButton variant="outlined" onClick={() => taskTypeHandler('completed')}>Completed</WhiteButton>
        )}
        <WhiteButton variant="outlined" onClick={() => setOpenDialog(true)}>Add Task</WhiteButton>
      </ButtonsContainer>
        ...
        {filteredTasks.length > 0 ? (
          filteredTasks.map((task) => (
            <div className="task-item" key={task.id}>
              <h1 style={{ color: "white", fontWeight: "300", fontSize: "26px" }}>{task.todo}</h1>
              <div className="task-buttons">
                {!task.completed && (
                  <WhiteButton variant="outlined" onClick={() => handleMarkAsDone(task.id)}>Done</WhiteButton>
                )}
                <WhiteButton variant="outlined" onClick={() => openEditDialogHandler(task)}>Edit</WhiteButton>
                <WhiteButton variant="outlined" onClick={() => handleDeleteTask(task.id)}>Delete</WhiteButton>
              </div>
            </div>
          ))
        ) : (
